...
let of_json : JSON.t -> Tac2val.valexpr = fun json ->
  let rec of_json json =
    let of_block n v = of_block (n, [|v|]) in
    match json with
    | `Null         -> of_int 0
    | `Bool(b)      -> of_block 0 (of_bool b)
    | `Int(i)       -> of_block 1 (of_int i)
    | `Intlit(s)    -> of_block 2 (of_string s)
    | `Float(f)     -> of_block 3 (of_float (Float64.of_float f))
    | `String(s)    -> of_block 4 (of_string s)
    | `Assoc(l)     -> of_block 5 (of_list (of_pair of_string of_json) l)
    | `List(l)      -> of_block 6 (of_list of_json l)
    | `Tuple(l)     -> of_block 7 (of_list of_json l)
    | `Variant(s,o) ->
        of_block 8 (of_pair of_string (of_option of_json) (s,o))
  in
  of_json json
...
let to_json : Tac2val.valexpr -> JSON.t = fun v ->
  let rec to_json v =
    ...
    | (8, [|v|]) -> `Variant(to_pair to_string (to_option to_json) v)
    | (_, _    ) -> assert false
  in
  to_json v
