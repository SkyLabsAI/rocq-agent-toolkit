...
def _test_roundtrip[T](
    ...
    **kwargs,
    ...
    assert data == deserialize_str(rat_utils.json.dumps(data, **kwargs))
    assert data == deserialize_structured(
        rat_utils.json.dumps(data, structured=True, **kwargs)
    )
...
def test_dumps_serialization_error():
    with pytest.raises(TypeError) as excinfo:
        rat_utils.json.dumps(test_dumps_serialization_error)
    assert re.search(
