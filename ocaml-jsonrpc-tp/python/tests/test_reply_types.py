...
class ReplyFixtures:
    type _mk_Reply[T] = Callable[[T], API.Reply[T]]
    type _mk_Err[T] = Callable[[str, T], API.Err[T]]
    type _mk_Resp[T] = Callable[[T], API.Resp[T]]

    @pytest.fixture(scope="session")
    @staticmethod
    def mk_Reply[T]() -> _mk_Reply[T]:
        return lambda data: API.Reply(data=data)

    @pytest.fixture(scope="session")
    @staticmethod
    def mk_Err[T]() -> _mk_Err[T]:
        return lambda message, data: API.Err(message=message, data=data)

    @pytest.fixture(scope="session")
    @staticmethod
    def mk_Resp[T]() -> _mk_Resp[T]:
        return lambda result: API.Resp(result=result)
    ...
    @pytest.fixture
    @staticmethod
    def Reply_None(mk_Reply: _mk_Reply[None]) -> API.Reply[None]:
        return mk_Reply(None)

    @pytest.fixture
    @staticmethod
    def Err_None(mk_Err: _mk_Err[None]) -> API.Err[None]:
        return mk_Err("", None)

    @pytest.fixture
    @staticmethod
    def Resp_None(mk_Resp: _mk_Resp[None]) -> API.Resp[None]:
        return mk_Resp(None)
...
class TestReplyErrResp(ReplyFixtures):
    ...
    def test_bool_Reply_not_implemented(self, Reply_None: API.Reply[None]) -> None:
        with pytest.raises(NotImplementedError):
            bool(Reply_None)

    def test_bool_Err(self, Err_None: API.Err[None]) -> None:
        assert not bool(Err_None)

    def test_bool_Resp(self, Resp_None: API.Resp[None]) -> None:
    ...
    def test_covariant_Reply(
        ...
        r3: API.Reply[Exception] = mk_Reply(Exception())

        assert r1 != r2
        assert r1 != r3
        assert r2 != r3

        def rs() -> Sequence[API.Reply[None | str | Exception]]:
            return [r1, r2, r3]

        _r: API.Reply[None | str | Exception]
        for r in rs():
            _r = r
            assert _r == r
