...
class HttpExtractor(AttributeExtractor):
    ...
    def extract_attributes(self, func: Callable, args: Tuple, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # URL information
        url = getattr(request, 'url', None) or getattr(request, 'build_absolute_uri', lambda: None)()
        if url:
        ...
        # Scheme
        scheme = getattr(request, 'scheme', None) or getattr(request, 'is_secure', lambda: None)()
        if scheme is True:
