...
class DatabaseExtractor(AttributeExtractor):
    ...
    def _infer_operation(self, func: Callable) -> Optional[str]:
        ...
        if any(op in func_name for op in ['create', 'insert', 'add', 'save']):
            return "insert"
        elif any(op in func_name for op in ['get', 'find', 'select', 'query', 'fetch', 'read']):
            return "select"
    ...
    def _extract_query_info(self, args: Tuple, kwargs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _extract_connection_info(self, args: Tuple, kwargs: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if hasattr(arg, 'get_dsn_parameters'):
                ...
                try:
                    ...
                    if 'host' in dsn_params:
                        attrs["db.connection_string"] = f"postgresql://{dsn_params['host']}"
