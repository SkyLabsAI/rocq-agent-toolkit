...
class ObservabilityConfig(LoggingConfig):
    ...
    def from_environment(cls, **overrides: Any) -> "ObservabilityConfig":
        ...
        env_config: dict[str, Any] = {
            ...
            "enable_tracing": os.getenv("OTEL_ENABLE_TRACING", "true").lower()
            == "true",
            "enable_metrics": os.getenv("OTEL_ENABLE_METRICS", "true").lower()
            == "true",
            "enable_logging": os.getenv("OTEL_ENABLE_LOGGING", "true").lower()
            == "true",
            "enable_otlp_log_export": os.getenv(
                "OTEL_ENABLE_OTLP_LOG_EXPORT", "true"
            ).lower()
            == "true",
            "default_extractor": os.getenv("OTEL_DEFAULT_EXTRACTOR"),
            "trace_sampling_rate": float(os.getenv("OTEL_TRACE_SAMPLING_RATE", "1.0")),
            "enable_langsmith": os.getenv("OTEL_ENABLE_LANGSMITH", "true").lower()
            == "true",
            "langchain_tracing_v2": os.getenv("LANGCHAIN_TRACING_V2", "false").lower()
            == "false",
            "langsmith_service_suffix": os.getenv(
                "OTEL_LANGSMITH_SERVICE_SUFFIX", "langsmith"
            ),
        }
        resource_attrs = {}
    ...
    def validate(self) -> None:
        ...
        if not self.service_name or self.service_name == "unknown-service":
            raise ValueError(
                "service_name must be provided and cannot be 'unknown-service'"
            )
