...
let to_json env evd : Ltac_plugin.Tacexpr.raw_tactic_expr -> json =
  let rec to_json tac =
    let default () =
      `String (Pp.string_of_ppcmds (Ltac_plugin.Pptactic.pr_raw_tactic env evd tac))
    in
    ...
    let tag, args =
      ...
      | TacThens3parts (a, bs, b, bs2) ->
        ( "Thens3"
        , [ to_json a; `List (to_json_array bs); to_json b; `List (to_json_array bs2) ] )
      ...
      | TacIfThenCatch (a, b, c) -> "IfThenCatch", [ to_json a; to_json b; to_json c ]
      ...
      | TacSelect (goal, tac) -> "Select", [ goal_select_json goal; to_json tac ]
      | _ -> "Atom", [ default () ]
    in
