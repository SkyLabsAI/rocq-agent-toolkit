...
class ProofState:
    ...
    rdm_api.ProofState exposing a structured / hierarchical view of
    the focused goals.
    """

    def __init__(
        self,
        pf_state: rdm_api.ProofState | None,
        goal_ty_upperbound: type[RocqGoal] = RocqGoal,
    ) -> None:
        if pf_state is not None and not isinstance(pf_state, rdm_api.ProofState):
            raise ValueError(
                " ".join(
                    [
        ...
        if pf_state is not None:
            ...
            # NOTE: we avoid making a shallow copy because `rdm_api.ProofState` is a frozen
            ...
            for rel_goal_num, focused_goal_str in enumerate(
                ...
                self._focused_goals[rel_goal_num] = parse.str_into_Goal(
                    ...
                    # NOTE: we don't get this from rdm_api.ProofState
                    rocq_goal_id=None,
                )
