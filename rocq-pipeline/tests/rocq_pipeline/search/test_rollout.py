...
def test_interleave_delayed() -> None:
    def tester() -> Generator[Rollout.Approx[int]]:
        yield Rollout.Approx(log(0.3), None)
    ...
    assert ir.next(log(0.6)) == Rollout.Approx(log(0.5), None)
    assert ir.next(log(0.5)) == Rollout.Approx(log(0.5), 1)
...
def test_interleave_1() -> None:
    ...
    assert ir.next(log(0.5)) == Rollout.Approx(log(0.1), None)
    assert ir.next(log(0.6)) == Rollout.Approx(log(0.1), None)
    assert ir.next(log(0.1)) == Rollout.Approx(log(0.1), 1)
    is_empty(ir)
...
def test_interleave_rollout() -> None:
    ...
    assert ir.next(log(0.5)) == Rollout.Approx(log(0.2), None)
    assert ir.next(log(0.6)) == Rollout.Approx(log(0.2), None)
    assert ir.next(log(0.2)) == Rollout.Approx(log(0.2), 2)
    assert ir.next(log(0.2)) == Rollout.Approx(log(0.1), None)
    assert ir.next(log(0.1)) == Rollout.Approx(log(0.1), 1)
    is_empty(ir)
