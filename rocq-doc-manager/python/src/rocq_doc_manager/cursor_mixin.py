...
class CursorMixin(RocqCursorProtocol):
    ...
    def text_query(self, text: str, index: int) -> str | API.Err[API.CommandError]:
        return self.query_text(text, index)

    @deprecated("use `query_text_all`")
    def text_query_all(
        self, text: str, indices: list[int] | None
    ) -> list[str] | API.Err[API.CommandError]:
        return self.query_text_all(text, indices)

    @deprecated("use `query_json`")
    def json_query(self, text: str, index: int) -> Any | API.Err[API.CommandError]:
    ...
    def insert_command(
        ...
        insert_reply = self.insert_command(text)
    ...
    def advance_to_first_match(
        ...
        def pretty_match_candidates(
            candidates: Sequence[tuple[int, API.PrefixItem | API.SuffixItem]],
        ) -> str:
            return "\n".join(
                [
