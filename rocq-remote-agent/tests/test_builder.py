...
def test_builder_uses_cached_or_env_github_token(monkeypatch: pytest.MonkeyPatch) -> None:
    import importlib
    ...
    monkeypatch.setattr(builder_mod, "resolve_github_token", lambda **_kw: "gh-from-env")
    monkeypatch.setattr(
        builder_mod,
    ...
    assert built._config.inference == {"provider": "openrouter", "api_key": "ok-api-key"}
