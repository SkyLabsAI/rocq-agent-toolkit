...
export const SlidingTabs: React.FC<SlidingTabsProps> = ({
  ...
  return (
    <div className={` relative rounded-sm w-fit p-[3px] border border-background-accent-gray-subtlest bg-elevation-surface-raised`}>
      <div 
        ref={containerRef}
      ...
      >
        {tabs.map((tab) => (
          ...
          >
            {tab.icon && (
              <span className={activeTab === tab.id ? "text-text" : "text-text-disabled"}>
                {tab.icon}
              </span>
            )}
            <span
