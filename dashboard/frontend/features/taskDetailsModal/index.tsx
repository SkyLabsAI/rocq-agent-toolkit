...
const TaskDetailsModal: React.FC<TaskDetailsModalProps> = ({
  ...
  const renderCustomContent = (key: string, value: unknown) => {
    ...
    if (
      ...
      lowerKey.includes('lemma') ||
      lowerKey.includes('states')
    ) {
      return (
        <div className='space-y-4'>
  ...
  const renderJsonContent = (key: string, value: unknown) => {
    ...
    return (
      <div className='bg-elevation-surface-raised border border-elevation-surface-overlay rounded-lg p-4 max-h-96 overflow-auto'>
        <pre className='text-sm text-text whitespace-pre-wrap font-mono'>
          {jsonString}
        </pre>
      </div>
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={title} size='full'>
      <div className='space-y-4'>
        ...
        {availableKeys.length > 0 && (
          <>
            {/* Tab Navigation */}
            <div className='flex flex-wrap border-b border-elevation-surface-overlay'>
